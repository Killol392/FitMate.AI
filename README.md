...
In the recent update I have added several new functionalities to improve the accuracy and the overall user experience of the system:

(1) Introduced alerts when the user performs repetitions too fast or outside the recommended tempo, so that the system can maintain proper form and avoid incorrect posture execution.

(2) Added an interface that allows the user to easily select which exercise they want to perform before the tracking begins. This gives more flexibility and better structure to the workflow instead of running exercises individually.

(3) Added an automatic shut-off mechanism that stops the program if no user is detected for 30 minutes. This prevents unnecessary processing and ensures device safety when left unattended.

(4) The system now records which exercise the user performs, how long they performed it, and how many repetitions they completed. All this data is saved for future use, including upcoming machine learning updates that will learn patterns and improve personalized recommendations over time.
